...
function Card({
  ...
  return (
    <div
      ...
      className={cn(
        "group/card flex flex-col gap-4 overflow-hidden rounded-xl bg-card py-5 text-sm text-card-foreground shadow-sm has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-3 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-xl *:[img:last-child]:rounded-b-xl",
        variantBorder,
        className
      )}
      {...props}
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-xl border-t bg-accent/50 p-5 group-data-[size=sm]/card:p-3",
        className
      )}
      {...props}
    />
